...
public class WealthDecisionController {
    ...
    public Map<String, List<WealthWarningInfo>> getWealthWarningInfos(long time, int granularity, int categories, String attributes,String corporation) {
        List<WealthWarningInfo> warningInfos = getWarningInfos(time, granularity, categories, attributes);
        for (int i = 0; i < warningInfos.size(); i++) {
            WealthWarningInfo info = warningInfos.get(i);
            for(int j = i + 1; j < warningInfos.size(); ) {
                WealthWarningInfo info2 = warningInfos.get(j);
                if(judge(granularity, info, info2)) {
                    warningInfos.remove(info2);
                }else j++;
            }
        }
    ...
    @GetMapping("/warning/query2")
    public Map<String, List<WealthWarningInfo>> getWealthWarningInfos2(long time, int granularity, int categories, String attributes,String corporation) {
        List<WealthWarningInfo> warningInfos = getWarningInfos(time, granularity, categories, attributes);
        return warningInfos.stream().collect(Collectors.groupingBy(WealthWarningInfo::getCorporation));
    }

    @GetMapping("/measure/query2")
    public Map<String, List<WealthMeasureInfo>> getWealthMeasureInfos2(long time, int granularity, int categories, String attributes) {
        List<WealthWarningInfo> warningInfos = getWarningInfos(time, granularity, categories, attributes);
    ...
    private boolean judge(int granularity, WealthWarningInfo info, WealthWarningInfo info2) {
        boolean flag = info.getCorporation().equals(info2.getCorporation()) && info.getAlarmType() == info2.getAlarmType() && info.getLevel() == info2.getLevel()
                && info.getCauseType().equals(info2.getCauseType());
        if(granularity == 3) flag = flag && TimeStampUtils.isSameDay(info.getEventTime(), info2.getEventTime());
        else if(granularity == 1) flag = flag &&TimeStampUtils.isSameQuarter(info.getEventTime(), info2.getEventTime());
        else if(granularity == 2) flag = flag &&TimeStampUtils.isSameMonth(info.getEventTime(), info2.getEventTime());
        if( flag) {
            info.setDevice(info.getDevice().add(info2.getDevice()));
            info.setFinance(info.getFinance().add(info2.getFinance()));
            info.setMateriel(info.getMateriel().add(info2.getMateriel()));
            info.setProduction(info.getProduction().add(info2.getProduction()));
            info.setProfit(info.getProfit().add(info2.getProfit()));
            info.setResearch(info.getResearch().add(info2.getResearch()));
            info.setSalary(info.getSalary().add(info2.getSalary()));
            info.setStorage(info.getStorage().add(info2.getStorage()));
            info.setTransportation(info.getTransportation().add(info2.getTransportation()));
            info.setCashAssets(info.getCashAssets().add(info2.getCashAssets()));
            info.setRevenue(info.getRevenue().add(info2.getRevenue()));
            info.setFixedAssets(info.getFixedAssets().add(info2.getFixedAssets()));
    }
        return flag;
    }
        ...
        for (Wealth wealth : wealths) {
            ...
            if (threshold != null) {
                ...
                try {
                    ...
                    warningInfo.setEventTime(wealth.getEventTime());
