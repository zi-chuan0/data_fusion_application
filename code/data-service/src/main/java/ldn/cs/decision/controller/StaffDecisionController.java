...
public class StaffDecisionController {
    ...
    public Map<String, List<StaffWarningInfo>> getStaffWarningInfos(long time, int granularity, int categories, String attributes) {
        ...
        for (int i = 0; i < warningInfos.size(); i++) {
            StaffWarningInfo info = warningInfos.get(i);
            for(int j = i + 1; j < warningInfos.size(); ) {
                StaffWarningInfo info2 = warningInfos.get(j);
                boolean flag;
                if(granularity == 1) flag = TimeStampUtils.isSameQuarter(info.getEventTime(),info2.getEventTime());
                else if(granularity ==2) flag = TimeStampUtils.isSameMonth(info.getEventTime(),info2.getEventTime());
                else flag = TimeStampUtils.isSameDay(info.getEventTime(),info2.getEventTime());
                if(flag && info.getCauseType().equals(info2.getCauseType())&& info.getCorporation().equals(info2.getCorporation()) && info.getAlarmType() == info2.getAlarmType() && info.getLevel() == info2.getLevel() &&
                info.getPositions() == info2.getPositions() &&
                info.getSkill().equals(info2.getSkill())) {
                    info.setAmount(info.getAmount() + info2.getAmount());
    ...
    public Map<String, List<StaffWarningInfo>> getStaffWarningInfos2(long time, int granularity, int categories, String attributes) {
        List<StaffWarningInfo> warningInfos = getWarningInfos(time, granularity, categories, attributes);
        return warningInfos.stream().collect(Collectors.groupingBy(StaffWarningInfo::getCorporation));
    }

    @GetMapping("/measure/query2")
    public Map<String, List<StaffMeasureInfo>> getStaffMeasureInfos2(long time, int granularity, int categories, String attributes) {
        List<StaffWarningInfo> warningInfos = getWarningInfos(time, granularity, categories, attributes);
    ...
    public Map<String, List<StaffMeasureInfo>> getStaffMeasureInfos(long time, int granularity, int categories, String attributes) {
        ...
        for (int i = 0; i < warningInfos.size(); i++) {
            ...
            for(int j = i + 1; j < warningInfos.size(); ) {
                ...
                if(flag && info.getCorporation().equals(info2.getCorporation())&&info.getAlarmType() == info2.getAlarmType() && info.getLevel() == info2.getLevel() &&
                        info.getPositions() == info2.getPositions()  &&
                        info.getSkill().equals(info2.getSkill())&& info.getCauseType().equals(info2.getCauseType())) {
                    info.setAmount(info.getAmount() + info2.getAmount());
                    warningInfos.remove(info2);
                }else j++;
            }
    ...
    private List<StaffWarningInfo> getWarningInfos(long time, int granularity, int categories, String attributes) {
        ...
        for (Staff staff : staffs) {
            ...
            if (threshold != null) {
                ...
                warningInfo.setEventTime(staff.getEventTime());
