...
public class ConveyDecisionController {
    ...
    public Map<String, List<ConveyWarningInfo>> getConveyWarningInfos(long time, int granularity, int categories, String attributes) {
        ...
        for(int i = 0; i < warningInfos.size();i++) {
            ConveyWarningInfo info = warningInfos.get(i);
            for(int j = i + 1; j < warningInfos.size(); ) {
                ConveyWarningInfo info2 = warningInfos.get(j);
    ...
    public Map<String, List<ConveyWarningInfo>> getConveyWarningInfos2(long time, int granularity, int categories, String attributes) {
        List<ConveyWarningInfo> warningInfos = getWarningInfos(time, granularity, categories, attributes);
        return warningInfos.stream().collect(Collectors.groupingBy(ConveyWarningInfo::getCorporation));
    }

    @GetMapping("/measure/query2")
    public Map<String, List<ConveyMeasureInfo>> getConveyMeasureInfos2(long time, int granularity, int categories, String attributes) {
        List<ConveyWarningInfo> warningInfos = getWarningInfos(time, granularity, categories, attributes);
    ...
    boolean judge(int granularity, ConveyWarningInfo info, ConveyWarningInfo info2) {
        ...
        if(flag) {
            ...
            info.setMileage(info.getMileage().add(info2.getMileage()));
        ...
        for (Convey convey : conveys) {
            ...
            if (threshold != null) {
                ...
                try {
                    ...
                    warningInfo.setEventTime(convey.getEventTime());
