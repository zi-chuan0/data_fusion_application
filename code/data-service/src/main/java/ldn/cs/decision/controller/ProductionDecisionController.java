...
public class ProductionDecisionController {
    ...
    public Map<String, List<ProductionWarningInfo>> getProductionWarningInfos(long time, int granularity, int categories, String attributes) {
        ...
        for(int i = 0; i < warningInfos.size();i++) {
            ProductionWarningInfo info = warningInfos.get(i);
            for(int j = i + 1; j < warningInfos.size();) {
                ProductionWarningInfo info2 = warningInfos.get(j);
    ...
    public Map<String, List<ProductionWarningInfo>> getProductionWarningInfos2(long time, int granularity, int categories, String attributes) {
        List<ProductionWarningInfo> warningInfos = getWarningInfos(time, granularity, categories, attributes);
        return warningInfos.stream().collect(Collectors.groupingBy(ProductionWarningInfo::getCorporation));
    }

    @GetMapping("/measure/query2")
    public Map<String, List<ProductionMeasureInfo>> getProductionMeasureInfos2(long time, int granularity, int categories, String attributes) {
        List<ProductionWarningInfo> warningInfos = getWarningInfos(time, granularity, categories, attributes);
    ...
    boolean judge(int granularity, ProductionWarningInfo info, ProductionWarningInfo info2) {
        ...
        if(flag) {
            ...
            info.setQuality((info.getQuality()+(info2.getQuality()))/2);
        }
        return flag;
    }
        ...
        for (Production production : productions) {
            ...
            if (threshold != null) {
                ...
                try {
                    ...
                    warningInfo.setEventTime(production.getEventTime());
