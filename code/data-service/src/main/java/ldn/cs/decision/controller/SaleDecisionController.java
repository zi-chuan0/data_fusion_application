...
public class SaleDecisionController {
    private static Map<Integer, String> upperReason4Cost;
    @Autowired
    ...
    public Map<String, List<SaleWarningInfo>> getSaleWarningInfos(long time, int granularity, int categories, String attributes) {
        ...
        for(int i = 0; i < warningInfos.size();i++) {
            SaleWarningInfo info = warningInfos.get(i);
            for(int j = i + 1; j < warningInfos.size(); ) {
                SaleWarningInfo info2 = warningInfos.get(j);
    ...
    public Map<String, List<SaleWarningInfo>> getSaleWarningInfos2(long time, int granularity, int categories, String attributes) {
        List<SaleWarningInfo> warningInfos = getWarningInfos(time, granularity, categories, attributes);
        return warningInfos.stream().collect(Collectors.groupingBy(SaleWarningInfo::getCorporation));
    }

    @GetMapping("/measure/query2")
    public Map<String, List<SaleMeasureInfo>> getSaleMeasureInfos2(long time, int granularity, int categories, String attributes) {
        List<SaleWarningInfo> warningInfos = getWarningInfos(time, granularity, categories, attributes);
    ...
    boolean judge(int granularity, SaleWarningInfo info, SaleWarningInfo info2) {
        boolean flag =  info.getCauseType().equals(info2.getCauseType())&&info.getAlarmType() == info2.getAlarmType() && info.getLevel() == info2.getLevel() &&
                info.getCorporation().equals(info2.getCorporation()) && info.getCategories() == info2.getCategories();
        ...
        if(flag) {
            info.setIncome(info.getIncome().add(info2.getIncome()));
            info.setCost(info.getCost().add(info2.getCost()));
            info.setQuantity(info.getQuantity().add(info2.getQuantity()));
            info.setInventory(info.getInventory().add(info2.getInventory()));
            info.setScore((info.getScore()+(info2.getScore()))/2);
        }
        return flag;
    }
        ...
        for (Sale sale : sales) {
            ...
            if (threshold != null) {
                ...
                try {
                    ...
                    warningInfo.setEventTime(sale.getEventTime());
